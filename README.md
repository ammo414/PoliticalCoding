...
#### Airflow
Airflow is cool. That's really about all there is to say here. I've used chron jobs in the past, but with anything even remotely complex, a DAG is almost necessarily the way to go. Being able to query data only after ensuring that the database is properly set up and bein able to painlessly assign tasks to be run in parallel is a great boon that ensures data safety.
